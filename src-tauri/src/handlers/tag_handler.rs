...
async fn generate_tags_html(app_handle: &AppHandle, tag_data: &[TagData], roll_width: &str) -> Result<String> {
    ...
    let template_path = if cfg!(debug_assertions) {
        ...
        let mut base_path = app_handle.path_resolver().app_local_data_dir()
            .ok_or_else(|| anyhow::anyhow!("Failed to get app local data directory"))?;
        
        // Adjust for the observed installation path
        if base_path.ends_with("com.uclean.app") {
            base_path.pop(); // Remove "com.uclean.app"
            base_path.push("UCLEAN"); // Add "UCLEAN"
        }

        base_path.join("src").join("templates").join("tags").join(template_file_name)
    };
